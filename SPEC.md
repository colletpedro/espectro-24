...
>
> Responda APENAS com JSON puro no formato: `{"narrativa": "<seu texto>", "consensos_usados": [{"propriedade": "<nome curto da propriedade descritiva>", "grupos_de_origem": ["<negativas|medianas|positivas>", ...], "temas_de_origem": ["<nome EXATO do tema, copiado do relatório>", ...]}], "quantificadores_usados": [{"quantificador": "<a expressão de frequência EXATA que você escreveu na prosa>", "tema": "<nome EXATO do tema, copiado do relatório, de onde ela vem>"}]}`. `consensos_usados` pode ser `[]` se o MOVIMENTO 2 não usou nenhuma propriedade consensual (ver OMISSÃO AUTORIZADA); `quantificadores_usados` pode ser `[]` se a prosa não quantificou nenhum tema.
